...
broad_sig <- sim_resonances(freq = 1.6, amp = 150, lw = 100, lg = 1,
                                phase = 0)
...
ed_ppm_end   <- 25
ed_ppm_N     <- 10
ed_ppm_vec   <- 10 ^ (seq(log10(ed_ppm_start), log10(ed_ppm_end),
                          length.out = ed_ppm_N))
...
fname <- "fig3_a.rds"        # precomputed results

if (file.exists(fname)) {  # don't recalc unless we have to
  cat("Reading precomputed results :", fname, "\n")
  res_list <- readRDS(fname) 
} else {
  res_list <- vector(mode = "list", length = ed_ppm_N)
  for (n in 1:ed_ppm_N) {
    
    
    opts  <- abfit_opts(auto_bl_flex = FALSE, ed_pppm = ed_ppm_vec[n],
...
error_vec        <- rep(NA, ed_ppm_N)
sd_error_vec     <- rep(NA, ed_ppm_N)
smo_res_fit_list <- vector(mode = "list", length = ed_ppm_N)
resid_list       <- vector(mode = "list", length = ed_ppm_N)
...
df <- data.frame(ed_ppm_vec, error_vec, sd_error_vec)

breaks <- c(2, 3, 5, 10, 25)

p1 <- ggplot(data = df, aes(x = ed_ppm_vec)) + 
...
df_ed      <- data.frame(ed_ppm_vec, res_ed_vec)
p2 <- ggplot(df_ed, aes(x = ed_ppm_vec, y = res_ed_vec)) + geom_line() +
...
cairo_pdf("fig3_a.pdf", width = 6.92, height = 5.5)
print(full_plot)
dev.off()
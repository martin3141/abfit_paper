...
Unprocessed MRS data typically have an unknown and erroneous frequency offset that displaces all resonances equally. The first step of ABfit is to correct the frequency offset to ensure the basis set of known signals are approximately matched to the acquired data. Raw MRS data is digitally sampled at a frequency of $\mathit{fs}$ Hz in the time-domain as a vector of $N$ complex data points:

\begin{equation}
  \mathbf{y}^{\mathrm{TD}} = \mathbf{y}(\mathbf{t}), \mathbf{t}=(t_{0}=0,t_{1} =1/\mathit{fs},\ldots,t_{N-1}=(N-1)/\mathit{fs}),
\end{equation}

where superscript TD and FD denote discrete frequency and time-domain representations respectively.

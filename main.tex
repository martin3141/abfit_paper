...
As with Equation \ref{p-spline_eq}, we regress the basis matrix on $y$ to yield $\alpha$, however since metabolite signals are always positive, we can improve analysis stability by enforcing a non-negative constraint on the $\alpha$ values corresponding to the weightings of $M$. The active-set method developed by Lawson and Hanson \cite{Lawson1995} is used herein to find the least-squares solution under the non-negative constraint --- while still allowing the $\alpha$ values corresponding to the spline basis to remain unconstrained.

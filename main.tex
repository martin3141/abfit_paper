...
\begin{equation}
  \mathbf{L}^{\mathrm{FD}} = \sqrt{\frac{4 \ln 2}{\pi}} \exp \left[ -4 \ln2 \left( \frac{\mathbf{f}}{\mathbf{l}(\mathbf{f})} \right) \right],
\end{equation}

\begin{equation}
  \mathbf{l}(\mathbf{f}) = \frac{2 d_{g}}{1 + \exp(a \ \mathbf{f})}.
\end{equation}

Note that when the asymmetry parameter $a=0$ the frequency dependence on the line-width is eliminated and $\mathbf{l}=d_{g}}$, resulting in pure Guassian broadening.
